use crate::entities::tb_ndms_map_water::ActiveModel;
use crate::entities::tb_ndms_map_water::Column;
use crate::entities::{prelude::*, tb_ndms_map_water};
use sea_orm::*;
  ...
  pub async fn saves(db: &DbConn, data: serde_json::Value) -> Result<InsertResult<ActiveModel>, DbErr> {
    ...
    if let serde_json::Value::Array(arr) = &data["list"] {
      for v in arr {
        ...
        let cd_dist_wal = v["cd_dist_wal"].as_i64().unwrap() as i32;
        c.set_from_json(v.clone()).expect("fail json value to active model");
        c.set(
          tb_ndms_map_water::Column::Flcode,
          sea_orm::Value::String(Some(Box::from(flcode.to_owned()))),
        );
        c.set(tb_ndms_map_water::Column::CdDistWal, sea_orm::Value::Int(Some(cd_dist_wal)));
        models.push(c);
      }
    }
    if models.len() == 0 {
    ...
    //TbNdmsMapWater::insert_many(models).exec(db).await
    TbNdmsMapWater::insert_many(models)
