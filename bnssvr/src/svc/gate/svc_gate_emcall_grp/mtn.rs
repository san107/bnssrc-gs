use crate::entities::{
  prelude::*,
  tb_gate_emcall_grp::{self, ActiveModel},
};
use sea_orm::*;
  ...
  pub async fn saves(db: &DbConn, data: serde_json::Value) -> Result<InsertResult<ActiveModel>, DbErr> {
    ...
    if let serde_json::Value::Array(arr) = &data["list"] {
      for v in arr {
        ...
        let emcall_seq = v["emcall_grp_seq"].as_i64().unwrap() as i32;

        c.set(tb_gate_emcall_grp::Column::GateSeq, sea_orm::Value::Int(Some(gate_seq)));
        c.set(
          tb_gate_emcall_grp::Column::EmcallGrpSeq,
          sea_orm::Value::Int(Some(emcall_seq)),
        );
        models.push(c);
      }
    }
    ...
    let rslt = TbGateEmcallGrp::insert_many(models)
