use crate::entities::{
  prelude::*,
  tb_gate_emcall::{self, ActiveModel},
};
use sea_orm::*;
  ...
  pub async fn saves(db: &DbConn, data: serde_json::Value) -> Result<InsertResult<ActiveModel>, DbErr> {
    ...
    if let serde_json::Value::Array(arr) = &data["list"] {
      for v in arr {
        ...
        let emcall_seq = v["emcall_seq"].as_i64().unwrap() as i32;

        c.set(tb_gate_emcall::Column::GateSeq, sea_orm::Value::Int(Some(gate_seq)));
        c.set(tb_gate_emcall::Column::EmcallSeq, sea_orm::Value::Int(Some(emcall_seq)));
        models.push(c);
      }
    }
    if models.len() == 0 {
    ...
    TbGateEmcall::insert_many(models)
