...
pub async fn do_cmd_up(
  ...
  let modbuscmd = pkt::get_hngsk_up_cmd();
  ...
  let rlt = loop {
    ...
    if stat == GateStatus::UpOk {
      // 성공.
      log::info!(
        "[HNGSK] UpOk rslt {rslt} stat {stat} msg {msg} elapsed {} secs",
        now.elapsed().as_secs()
      );
      send_cmd_res_all(&ctx, &cmd, rslt, stat, msg.clone()).await;
    ...
    log::info!(
      "[HNGSK] Current Status rslt {rslt} stat {stat} msg {msg} elapsed {} secs",
      now.elapsed().as_secs()
    );

    if now.elapsed().as_secs() > get_cmd_timeout_secs() {
      let msg = format!("[HNGSK] timeout elapsed {}", now.elapsed().as_secs());
      log::error!("{msg}");
      let rslt = GateCmdRsltType::Fail;
      let stat = GateStatus::Na;
      send_cmd_res_all(&ctx, &cmd, rslt, stat, msg.clone()).await;
