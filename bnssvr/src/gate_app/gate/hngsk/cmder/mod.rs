...
pub async fn get_status(
  ...
  if let Err(e) = data {
    let msg = format!("[HNGSK] read_holding_registers fail {e:?}{cmdmsg}");
    log::error!("{msg}");
  ...
  let (rslt, stat) = pkt::get_hngsk_stat(data);
  let statmsg = pkt::get_hngsk_stat_msg(data);
  log::debug!(
    "[HNGSK] addr {} val {} flags {}  {cmdmsg} {statmsg}",
    addr,
    data,
    pkt::parse(data).join(",")
  );
...
pub async fn do_cmd(ctx: &GateCtx, model: &tb_gate::Model, modbus: &mut Context, cmd: &GateCmd) -> anyhow::Result<DoGateCmdRslt> {
  ...
  let msg = format!("[HNGSK] Not Supported Cmd {:?}", cmd.cmd_type);
  log::error!("{msg}");
