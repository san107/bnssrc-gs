...
pub async fn get_status_changed(
  ...
  let data = modbus.read_input_registers(addr, 4).await;
  ...
  if let Err(e) = data {
    let msg = format!("[SYSBASE] read_input_registers fail {e:?}{cmdmsg}");
    log::error!("{msg}");
  ...
  let data = data.unwrap();
  log::debug!("[SYSBASE] addr {} val {:?}  {cmdmsg}", addr, data,);
...
pub async fn do_cmd(ctx: &GateCtx, model: &tb_gate::Model, modbus: &mut Context, cmd: &GateCmd) -> anyhow::Result<DoGateCmdRslt> {
  ...
  let msg = format!("[SYSBASE] Not Supported Cmd {:?}", cmd.cmd_type);
  send_cmd_res(
    &cmd,
    GateCmdRsltType::Fail,
    GateStatus::Na,
    format!("[SYSBASE] Not Supported Cmd {:?}", cmd.cmd_type),
  )
  .await;
