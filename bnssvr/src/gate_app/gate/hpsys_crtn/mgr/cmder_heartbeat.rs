...
use crate::gate_app::gate::hpsys_crtn::pkt::ADDR_HEARTBEAT;
use crate::gate_app::util::lock_write_single_register;
use crate::{eanyhowf, GateCtx};
use tokio_modbus::client::Context;

pub async fn do_hpsys_crtn_heartbeat(modbus: &mut Context, model: &tb_gate::Model) -> anyhow::Result<()> {
...
pub async fn mgr_do_heartbeat(_ctx: &GateCtx, model: &tb_gate::Model, modbus: &mut Context) {
  //
  let rlt = do_hpsys_crtn_heartbeat(modbus, &model).await;
  if let Err(e) = rlt {
    let msg = format!("[데몬] do_hpsys_crtn_water fail seq : {} {e:?}", model.gate_seq);
    log::error!("{msg}");
  }
  log::debug!("[데몬] do_hpsys_crtn_heartbeat success seq : {}", model.gate_seq);
}
