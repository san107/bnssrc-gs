...
pub async fn do_cmd_autodown(
  ...
  log::debug!("[HNGSK] start loop{} {} {}", cmd.cmd_type, cmd.gate_seq, model.gate_nm);
  let now = Instant::now();
  let rlt = loop {
    interval.tick().await;
    log::debug!("[HNGSK] start loop body {} {} {}", cmd.cmd_type, cmd.gate_seq, model.gate_nm);
    let (rslt, stat, msg) = super::get_status(ctx, addr, modbus, cmd, false).await;
    if rslt == GateCmdRsltType::Fail {
      let msg = format!(
        "[HNGSK] Fail {rslt} stat {stat} msg {msg}{cmdmsg} elapsed {} secs",
        now.elapsed().as_secs()
      );
      log::error!("{msg}");
    ...
    if stat == GateStatus::DownOk {
      // 성공.
      log::info!(
        "[HNGSK] DownOk rslt {rslt} stat {stat} msg {msg}{cmdmsg} elapsed {} secs",
        now.elapsed().as_secs()
      );
      let msg = format!("[HNGSK] {msg}{cmdmsg} elapsed {} secs", now.elapsed().as_secs());
      send_cmd_res_all(&ctx, &cmd, rslt, stat, msg.clone()).await;
