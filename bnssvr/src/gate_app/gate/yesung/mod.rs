...
pub async fn do_cmd(ctx: GateCtx, model: tb_gate::Model, cmd: GateCmd) -> anyhow::Result<DoGateCmdRslt> {
  ...
  if let Err(e) = modbus {
    let msg = flnf!("[yesung] 연결 에러 seq is {seq}{cmdmsg}");
  ...
  debug!("[yesung] addr is {modbus:?}{cmdmsg}");
  ...
  // if let Err(e) = data {
  //   let msg = flnf!("[yesung] 읽기 에러 seq is {seq}{cmdmsg}");
  //   let stat = GateStatus::Na;
  //   let rslt = GateCmdRsltType::Fail;
  //   if cmd.cmd_type != GateCmdType::Stat {
  //     send_cmd_res_changed(&ctx, &model, &cmd, rslt, stat, msg.clone()).await;
  ...
  // debug!("[yesung] read data is {:?} {} {cmdmsg}", data, vec_to_hex_u16(&data));
  // if data.len() > 0 {
  //   log::debug!(
  //     "[yesung] addr is {addr} data is {} {cmdmsg}",
