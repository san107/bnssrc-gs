...
use crate::gate_app::util::{lock_read_holding_registers, ws_send_gate_stat_all};
...
pub async fn mgr_get_status(
  ...
  let stat = super::super::pkt::get_hpsys_stat(data);
  log::debug!("[데몬] addr {} flags {}", addr, super::super::pkt::parse(data).join(","));

  let rslt = if super::super::pkt::get_hpsys_is_remote(data) {
