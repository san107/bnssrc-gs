...
pub async fn get_status(
  ...
  if let Err(e) = data {
    let msg = format!("[HPSYS] read_holding_registers fail {e:?}{cmdmsg}");
    log::error!("{msg}");
  ...
  let stat = pkt::get_hpsys_stat(data);
  let stat_str = pkt::get_hpsys_stat_str(data);
  log::debug!("[HPSYS] addr {} val {} flags {}  {cmdmsg}", addr, data, stat_str);

  let rslt = if pkt::get_hpsys_is_remote(data) {
...
pub async fn do_cmd(ctx: &GateCtx, model: &tb_gate::Model, modbus: &mut Context, cmd: &GateCmd) -> anyhow::Result<DoGateCmdRslt> {
  ...
  } else if cmd.cmd_type == GateCmdType::Side {
    return cmd_side::do_cmd_side(ctx, model, modbus, cmd).await;
  } else if cmd.cmd_type == GateCmdType::Center {
    return cmd_center::do_cmd_center(ctx, model, modbus, cmd).await;
  }

  let msg = format!("[HPSYS] Not Supported Cmd {:?}", cmd.cmd_type);
