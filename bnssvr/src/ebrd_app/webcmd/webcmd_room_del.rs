...
use crate::ebrd_app::model::EbrdWebRoomDel;
...
async fn do_webcmd_send(
  ...
  pkt.cmd = pkt::Cmd::RoomDel;
  pkt.id = model.ebrd_id.clone();

  let mut cmd = cmds::cmd_room_del::CmdRoomDel::new0();
  cmd.room_no = cmdinfo.ebrd_msg_pos;
...
pub async fn do_webcmd(db: DbConn, cmd: &mut EbrdWebRoomDel, tx_ws: broadcast::Sender<Box<String>>) -> Result<(), ()> {
  log::info!("EbrdWebRoomDel: {}", cmd.ebrd_seq);
  let tx = cmd.tx.take().unwrap();
  let rslt = do_webcmd_send(db.clone(), cmd).await;
  if rslt.is_err() {
    let (msg, model) = rslt.err().unwrap();
    log::error!("ebrd cmd room del error: {}", msg);
    ...
    if model.is_none() {
      let msg = "ebrd cmd room del error: model is none ";
      log::error!("{}", msg);
