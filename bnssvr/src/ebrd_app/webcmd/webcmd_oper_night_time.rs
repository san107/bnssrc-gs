...
use crate::ebrd_app::model::EbrdWebOperNightTime;
...
async fn do_webcmd_send(
  db: DbConn,
  cmdinfo: &mut EbrdWebOperNightTime,
  ...
  pkt.cmd = pkt::Cmd::OperNightTime;
  pkt.id = model.ebrd_id.clone();
  pkt.data = cmdinfo.cmd.to_bytes();
...
pub async fn do_webcmd(db: DbConn, cmd: &mut EbrdWebOperNightTime, tx_ws: broadcast::Sender<Box<String>>) -> Result<(), ()> {
  log::info!("EbrdWebOperNightTime: {}", cmd.ebrd_seq);
  let tx = cmd.tx.take().unwrap();
  let rslt = do_webcmd_send(db.clone(), cmd).await;
  if rslt.is_err() {
    let (msg, model) = rslt.err().unwrap();
    log::error!("ebrd cmd oper night time error: {}", msg);
    ...
    if model.is_none() {
      let msg = "ebrd cmd oper night time error: model is none ";
      log::error!("{}", msg);
    ...
    if cmd_err.is_some() {
      let _ = tx.send(Err(format!("{}", cmd_err.unwrap().err_code)));
    } else {
      let _ = tx.send(Ok(()));
    }
  }
