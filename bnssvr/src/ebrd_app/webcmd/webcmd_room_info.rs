...
use crate::ebrd_app::model::EbrdWebRoomInfo;
...
pub async fn do_webcmd(db: DbConn, cmd: &mut EbrdWebRoomInfo, tx_ws: broadcast::Sender<Box<String>>) -> Result<(), ()> {
  log::info!("EbrdWebRoomInfo: {}", cmd.ebrd_seq);
  let tx = cmd.tx.take().unwrap();
  let rslt = do_webcmd_send(db.clone(), cmd).await;
  if rslt.is_err() {
    let (msg, model) = rslt.err().unwrap();
    log::error!("ebrd cmd room info error: {}", msg);
    ...
    if model.is_none() {
      let msg = "ebrd cmd room info error: model is none ";
      log::error!("{}", msg);
