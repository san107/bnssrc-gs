...
pub async fn list(app: web::Data<crate::AppState>, req: HttpRequest) -> impl Responder {
  ...
  let ret = svc_gate_emcall::qry::Qry::find_by_id(&app.conn, seq).await;
  let obj = match ret {
    Ok(l) => l,
    _ => vec![],
  };
...
#[get("/api/gate_emcall/emcalllist")]
pub async fn emcalllist(_sess: Session, app: web::Data<crate::AppState>, req: HttpRequest) -> impl Responder {
  ...
  let rlt = svc_gate_emcall::qry::Qry::find_by_withemcall(&app.conn, seq).await;
  let l = match rlt {
    Ok(l) => l,
    _ => vec![],
  };
