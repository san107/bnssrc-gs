use crate::{
  ...
  svc::ndms::svc_ndms_map_water::{self, mtn::Mtn},
};
...
pub async fn list(app: web::Data<crate::AppState>, req: HttpRequest) -> impl Responder {
  ...
  if let Some(flcode) = flcode {
    let ret = svc_ndms_map_water::qry::Qry::find_by_flcode(&app.conn, flcode).await;
    let obj = match ret {
      Ok(l) => l,
      _ => vec![],
    };
  ...
  let ret = svc_ndms_map_water::qry::Qry::find_all(&app.conn).await;
  let obj = match ret {
    Ok(l) => l,
    _ => vec![],
  };
