use crate::svc::ndms::svc_flud_board::{self, mtn::Mtn};
...
pub async fn one(_sess: Session, app: web::Data<crate::AppState>, req: HttpRequest) -> impl Responder {
  ...
  let cd_dist_board = qs.get("cd_dist_board").unwrap().parse().unwrap();

  let ret = svc_flud_board::qry::Qry::find_by_id(&app.conn, (flcode, cd_dist_board)).await;
  let obj = match ret {
    Ok(l) => l,
    _ => None,
  };
...
pub async fn list(app: web::Data<crate::AppState>, req: HttpRequest) -> impl Responder {
  ...
  if let Some(flcode) = flcode {
    let ret = svc_flud_board::qry::Qry::find_by_flcode(&app.conn, flcode).await;
    let obj = match ret {
      Ok(l) => l,
      _ => vec![],
    };
  ...
  let rlt = svc_flud_board::qry::Qry::find_all(&app.conn).await;
  let l = match rlt {
    Ok(l) => l,
    _ => vec![],
  };
