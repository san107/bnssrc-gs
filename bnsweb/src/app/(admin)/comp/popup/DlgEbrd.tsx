...
type Props = {
  show: (props?: {}) => Promise<{ cmd: string; ebrdSeq?: number }>;
};

export const DlgEbrd = React.forwardRef<Props, unknown>((props, ref) => {
          ...
          >
            {(list || []).map((ele) => (
              <ListItemButton
                ...
                onClick={() => {
                  setOpen(false);
                  promise.current.resolve?.({ cmd: 'ok', ebrdSeq: ele.ebrd_seq });
                }}
              >
...
DlgEbrd.displayName = 'DlgEbrd';
export const useDlgEbrd = () => useRefComponent<Props>(DlgEbrd);
