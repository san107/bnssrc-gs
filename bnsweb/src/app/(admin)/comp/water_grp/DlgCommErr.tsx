...
export type IfCommErr = {};

export type IfDlgCommErr = {
  show: (props: { waterGrpId: string }) => Promise<IfCommErr>;
  close: () => void;
};

export const DlgCommErr = React.forwardRef<IfDlgCommErr, unknown>((props, ref) => {
  const [open, setOpen] = React.useState(false);
  const promise = usePromise<IfCommErr, { cmd: string; msg?: string }>();
  ...
  React.useImperativeHandle<IfDlgCommErr, IfDlgCommErr>(
    ref,
    (): IfDlgCommErr => ({
  ...
  return (
    <React.Fragment>
      <DlgBase onClose={handleClose} open={open} maxWidth={false} closeAfterTransition={false}>
        <DlgTitle title={'수위계 그룹 통신에러'} handleClose={handleClose} />
...
DlgCommErr.displayName = 'DlgCommErr';
export const useDlgCommErr = () => useRefComponent<IfDlgCommErr>(DlgCommErr);
