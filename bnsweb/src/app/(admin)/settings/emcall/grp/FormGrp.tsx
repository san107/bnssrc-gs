...
type Props = {
  sel: IfTbEmcallGrp;
  setSel: (v: IfTbEmcallGrp) => void;
};

export const FormGrp = ({ sel, setSel }: Props) => {
  const { mutate } = useSWRConfig();

  const handleReset = () => {
    setSel(new TbEmcallGrp());
  };

  const handleNewSave = () => {
    const copySel = { ...sel };
    delete copySel.emcall_grp_seq; // 일련번호 제거
    axios
      .post('/api/emcall_grp/save', copySel)
      .then((res) => {
        setSel(res.data);
  ...
  const handleUpdate = () => {
    if (!sel.emcall_grp_lat || !sel.emcall_grp_lng) {
      toast.error('위도와 경도를 선택해주세요');
      return;
    }
    axios
      .post('/api/emcall_grp/save', sel)
      .then((res) => {
        setSel(res.data);
  ...
  const handleDelete = () => {
    confirm('확인', ['삭제하시겠습니까'])
      ?.then(() => {
        console.log('삭제 확인. ');
        axios
          .post('/api/emcall_grp/delete', sel)
          .then((res) => {
  ...
  const handleClickLatLng = () => {
    const pos = (n?: number | null) => {
      if (n && n < 20) return undefined;
      if (n == null) return undefined;
      return n;
    }; //잘못된 위경도인 겨우, undefined로 리턴함.
    dlgMap.current
      ?.show('스피커 송출그룹 좌표 선택', {
        lat: pos(sel.emcall_grp_lat),
        lng: pos(sel.emcall_grp_lng),
      })
      .then((res) => {
        setSel({ ...sel, emcall_grp_lat: res.lat, emcall_grp_lng: res.lng });
        toast.info('선택 하였습니다');
      })
      .catch((rejection) => {
  ...
  return (
    <StyledCard>
              ...
              <FormTr>
                ...
                <FormTd sx={{ pr: 3 }}>
                  <TextField
                    fullWidth
                    size='small'
                    value={sel?.emcall_grp_nm || ''}
                    onChange={(e) => setSel({ ...sel, emcall_grp_nm: e.target.value })}
                  />
                </FormTd>
              </FormTr>
              <FormTr>
                ...
                <FormTd sx={{ pr: 3 }}>
                  <TextField
                    fullWidth
                    size='small'
                    value={sel?.emcall_grp_id || ''}
                    onChange={(e) => setSel({ ...sel, emcall_grp_id: e.target.value })}
                  />
                </FormTd>
              </FormTr>
              <FormTr>
                ...
                <FormTd sx={{ pr: 3 }} className='flex'>
                  <TextField
                    fullWidth
                    size='small'
                    value={sel?.emcall_grp_lat || ''}
                    onClick={handleClickLatLng}
                    placeholder='지도에서 선택하세요'
                  />
                  {/* {sel?.emcall_grp_lat ? (
                    <IconBtnDelete
                      onClick={() => setSel({ ...sel, emcall_grp_lat: null, emcall_grp_lng: null })}
                    />
                  ) : undefined} */}
                </FormTd>
              </FormTr>
              ...
              <FormTr>
                ...
                <FormTd sx={{ pr: 3 }}>
                  <TextField
                    fullWidth
                    size='small'
                    value={sel?.emcall_grp_ip || ''}
                    onChange={(e) => setSel({ ...sel, emcall_grp_ip: e.target.value })}
                  />
                </FormTd>
              </FormTr>
              <FormTr>
                ...
                <FormTd sx={{ pr: 3 }}>
                  <TextField
                    fullWidth
                    size='small'
                    value={sel?.emcall_grp_port || ''}
                    onChange={(e) =>
                      parseInt(e.target.value) &&
                      setSel({ ...sel, emcall_grp_port: parseInt(e.target.value) })
                    }
                  />
                </FormTd>
              </FormTr>
              ...
              <FormTr>
                ...
                <FormTd sx={{ pr: 3 }}>
                  <TextField
                    ...
                    value={sel?.emcall_tts_msg || ''}
                    onChange={(e) => setSel({ ...sel, emcall_tts_msg: e.target.value })}
                  />
                </FormTd>
              </FormTr>
              <FormTr>
      ...
      <StyledCardActions>
        ...
        <SettingBtn btnType='edit' onClick={handleUpdate} disabled={!sel?.emcall_grp_seq}>
          수정
        </SettingBtn>
        <SettingBtn btnType='reset' onClick={handleReset}>
          초기화
        </SettingBtn>
        <SettingBtn btnType='delete' onClick={handleDelete} disabled={!sel?.emcall_grp_seq}>
          삭제
        </SettingBtn>
      </StyledCardActions>
      <DlgMap />
